...
export default function SearchBar() {
  ...
  return (
    <div
      className={`w-80 h-8 flex relative items-center justify-center  rounded-md box-border ${
        isFocused ? "border-2 border-lime-300" : "border-2 border-transparent"
      }`}
    >
      <div className="flex bg-lime-800 w-28 h-full justify-center items-center text-center rounded-l-md">
        <Categories />
      </div>
      <div className="flex h-full flex-1">
        ...
        {searchParams.length > 0 && isAutofillOpen && (
          <SearchBarAutofillDropdown
            autofillResult={autofillResult}
            setIsAutofillOpen={setIsAutofillOpen}
          />
        )}
      </div>
    </div>
